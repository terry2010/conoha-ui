...
func Post(_url string, data string) (body []byte, err error) {
	return doRequest(RequestType.Post, timeout, retryTimes, _url, data)
}

func Get(_url string) (body []byte, err error) {
	return doRequest(RequestType.Get, timeout, retryTimes, _url, "")
}

func Delete(_url string) (body []byte, err error) {
...
DO_RETRY:
	...
	if nil != err {
		...
		log.Println("Http-doRequest-failed:", _url, err)
		if 0 < retryTimes {
...
func HttpPostForm(timeout time.Duration, retryTimes int, url string, data url.Values) (body []byte, err error) {
DO_RETRY:
	client, err := HttpClient(timeout)
	if nil != err {
		return nil, err
...
func HttpGetTCN2Long(timeout time.Duration, url string) (LongURL string, urlList []map[int]string, isSuccess bool, err error) {
